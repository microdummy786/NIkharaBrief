...
import { SaveIcon, ExportIcon, ErrorIcon } from './Icons';
...
export const BriefDisplay: React.FC<BriefDisplayProps> = ({ brief, isLoading, error, currentUser }) => {
        ...
        <div className="flex flex-col h-full animate-fade-in">
            ...
            <div className="flex flex-col sm:flex-row sm:justify-between sm:items-start gap-4 pb-4 border-b border-brand-border">
                ...
                <div className="flex items-center gap-2 flex-shrink-0 self-end sm:self-auto">
                    {isPaid && (
                        <button className="flex items-center gap-2 text-sm py-2 px-3 bg-brand-border hover:bg-white/10 transition-colors rounded-lg">
  ...
  return (
    <div className="w-full lg:w-3/5 bg-brand-bg-secondary/60 backdrop-blur-lg p-4 sm:p-6 lg:p-8 rounded-2xl border border-brand-border/50 min-h-[450px] sm:min-h-[500px] lg:min-h-[600px] flex flex-col">
        {renderBriefContent()}
    </div>
  );
};
