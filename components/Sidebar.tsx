...
import { Logo, SidebarToggleIcon, CloseIcon } from './Icons';
...
export const Sidebar: React.FC<SidebarProps> = ({ activePage, setActivePage, isCollapsed, setCollapsed, isMobileOpen, setMobileOpen, currentUser }) => {
  ...
  const NavLink: React.FC<{ item: { name: string, icon: React.FC<any> }, isProfile?: boolean }> = ({ item, isProfile = false }) => (
     <a
      href="#"
      onClick={(e) => { 
        e.preventDefault(); 
  ...
  return (
    <>
      <aside className={`fixed top-0 left-0 h-full bg-brand-bg-secondary/80 backdrop-blur-lg border-r border-brand-border flex flex-col transition-all duration-300 ease-in-out z-50 ${isCollapsed ? 'lg:w-20' : 'lg:w-64'} w-64 lg:translate-x-0 ${isMobileOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        <div className="flex items-center justify-between p-4 h-20 border-b border-brand-border flex-shrink-0">
          {isCollapsed ? (
            <div className="w-full flex justify-center">
              <button onClick={() => setCollapsed(!isCollapsed)} className="p-2 rounded-md hover:bg-white/10 hidden lg:block">
          ...
          <button onClick={() => setMobileOpen(false)} className={`p-2 rounded-md hover:bg-white/10 lg:hidden ${isCollapsed ? 'hidden': ''}`}>
        ...
        <div className="p-4 border-t border-brand-border flex-shrink-0 space-y-4">
            ...
            <div className="space-y-2">
                {PROFILE_NAV_ITEMS.map(item => (
                  <div key={item.name}>
                    <NavLink item={item} isProfile={true} />
                  </div>
                ))}
            </div>
        </div>
      </aside>
       {isMobileOpen && <div onClick={() => setMobileOpen(false)} className="fixed inset-0 bg-black/60 z-40 lg:hidden" aria-hidden="true" />}
    </>
  );
};